 ...
 *   1. Firestore — the `users/{uid}/tracker/*` subdocs and the `users/{uid}`
 *      profile document.
...
export const deleteAccount = onCall(callableOptions, async (request) => {
  ...
  // 1. Firestore — the tracker subcollection and the profile doc.
  try {
    const trackerSnap = await db.collection(`users/${uid}/tracker`).get();
    const batch = db.batch();
    trackerSnap.forEach((doc) => batch.delete(doc.ref));
